  ...
  return <main className="app-shell">
    <div className="dashboard">
      ...
      <section className="review-panel panel">
        ...
        <div className="review-table"><div className="review-head"><span>Gerencia / Área / División</span><span>Ceses no deseados</span><span>Rotación</span><span>{isSingleMonth ? "Variación mensual" : "Variación vs. rango anterior"}</span></div>{reviewAreas.length ? reviewAreas.map((item) => <article key={item.key} className="review-row"><div><strong>{item.area}</strong><small>{item.subArea} · {item.division} · {item.priority} · impacto {item.impactPoints.toFixed(2)} pp</small></div><span>{item.exits}</span><span><b className={item.smallBase ? "review-neutral" : item.rate > MONTHLY_TARGET ? "review-alert" : "review-ok"}>{item.rate.toFixed(2)}%</b></span><span>{item.variation === null ? <b className="review-neutral">—</b> : <b className={item.variation > 0 ? "review-alert" : "review-ok"}>{item.variation > 0 ? "↗" : item.variation < 0 ? "↘" : "→"} {Math.abs(item.variation).toFixed(2)} pp</b>}</span></article>) : <p className="empty-review">No hay ceses por decisión del trabajador en el último mes seleccionado.</p>}</div>
      </section>

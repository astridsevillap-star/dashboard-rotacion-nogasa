...
type IncomingRow = { y:number;m:number;a:string;d:string;r:string;q:string;h:number;i:number;c:number;v:number;x:number;d3:number;d6:number };
...
export async function GET() {
  try {
    await ensureSchema();
    const sql = database();
    const rows = await sql`SELECT year AS y, month AS m, area AS a, dotacion AS d, region AS r, category AS q, headcount AS h, hires AS i, exits AS c, employee AS v, company AS x, desert3 AS d3, desert6 AS d6 FROM uploaded_units ORDER BY year, month, area, region, category`;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const queries = rows.map((row) => {
      ...
      return sql`INSERT INTO uploaded_units (id, year, month, area, dotacion, region, category, headcount, hires, exits, employee, company, desert3, desert6, uploaded_at, source_name)
        VALUES (${id}, ${row.y}, ${row.m}, ${row.a}, ${row.d}, ${row.r}, ${row.q}, ${row.h}, ${row.i}, ${row.c}, ${row.v}, ${row.x}, ${row.d3}, ${row.d6}, ${uploadedAt}, ${sourceName})
        ON CONFLICT (id) DO UPDATE SET headcount=EXCLUDED.headcount, hires=EXCLUDED.hires, exits=EXCLUDED.exits, employee=EXCLUDED.employee, company=EXCLUDED.company, desert3=EXCLUDED.desert3, desert6=EXCLUDED.desert6, uploaded_at=EXCLUDED.uploaded_at, source_name=EXCLUDED.source_name`;
    });
